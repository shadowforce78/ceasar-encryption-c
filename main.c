...
int main(int argc, char *argv[])
{
    ...
    char text[1000]; // Variable pour le texte
    ...
    decalage = decalage % 26; // Calcul du decalage modulo 26
    ...
    { // Boucle pour lire le fichier 1
        if (c >= 'A' && c <= 'Z')
        {                     // Si le caractere est une majuscule
            c = c + decalage; // On ajoute le decalage
            if (c > 90)
            {               // Si le caractere depasse Z
                c = c - 26; // On soustrait 26 pour boucler
            }
        }
        else if (c >= 'a' && c <= 'z')
